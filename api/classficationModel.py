...
class Model :
    ...
    def trainAllData(self) :
        ...
        for i in range(len(dataSet)):
            ...
            if (i == 0):
                pythaiWord = pd.DataFrame({"word":pd.DataFrame(pythaiToken, columns = ["word"]).word.unique(), "genre": dataSet[i][3]})
            else:
                pythaiWord = pd.concat([pythaiWord, pd.DataFrame({"word":pd.DataFrame(pythaiToken, columns = ["word"]).word.unique(), "genre": dataSet[i][3]})])
    ...
    def predictData(self, text) :
        ...
        pythai_word = pd.DataFrame({"word":pd.DataFrame(pythai_token, columns = ["word"]).word.unique()})
    ...
    def updateFeature(self, category, wordList) :
        ...
        feature = "UPDATE feature SET " + catList[category-2] + " = " + catList[category-2] + " + 1, total = total + 1 WHERE word = %s"
        
        for word in wordList:
            cursor.execute(feature, [word])

        ficNumber = "UPDATE fiction_number SET " + catList[category-2] + " = " + catList[category-2] + " + 1, total = total + 1"
